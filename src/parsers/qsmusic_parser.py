...
class QSMusicParser(BaseParser):
    ...
    def __init__(self, real_url):
        ...
        self.cookie = get_platform_cookie("qishui")
        if self.cookie:
    ...
    def _parse_page(self):
        ...
        if not self.title or not (self.video_url or self.audio_url) or (self.is_preview and self.cookie):
    ...
    def _parse_track_payload(self):
        """走 Web 播放接口兜底；配置 QISHUI_COOKIE 时用登录态取完整音频。"""
